...
class Common {
  ...
  async scheduleNew(ctx) {
    ...
    try {
      await commonService.scheduleNewByFormulaType(client, formulaType, nextDate, productTypeId)
      ...
      if(formulaType == 'housing_metal' || formulaType == 'housing_plastic') {
        await commonService.scheduleNewByFormulaType(client, formulaType, nextDate, null)
      }
      client.commit()
      ctx.status = 200
    } catch(err) {
