...
class Rubber {
  ...
  async postMachineRate(ctx) {
    try {
      let { machineRate, ...params } = ctx.request.body

      if (!machineRate || machineRate == '') {
        throwApiError('not found request data', errorCode.NOT_FOUND)
      }

      let result = await rubberService.postMachineRate(machineRate, params)

      ctx.body = result
      ctx.status = 200
    } catch(err) {
      logger.warn('error request for Database Rubber postMachineRate function', err)
      throwApiError(err.message, err.status)
    }
  }
  async archiveMachineRate(ctx) {
    try {
      let { machineRateId } = ctx.request.body

      if (machineRateId.length <= 0) {
        throwApiError('not found request data', errorCode.NOT_FOUND)
      }

      let result = await rubberService.archiveMachineRate(machineRateId)

      ctx.body = result
      ctx.status = 200
    } catch(err) {
      logger.warn('error request for Database Rubber archiveMachineRate function', err)
      throwApiError(err.message, err.status)
    }
  }
  async unblockMachineRate(ctx) {
    try {
      let { machineRateId } = ctx.request.body
      ...
      let result = await rubberService.unblockMachineRate(machineRateId)

      ctx.body = result
      ctx.status = 200
    } catch(err) {
      logger.warn('error request for Database Rubber unblockMachineRate function', err)
      throwApiError(err.message, err.status)
    }
  }
}
