...
module.exports = {
  ...
  getItemsByVerId: async (client, id) => {
    let sql = squel.select()
      ...
      .left_join('wiprocurement.bom_stage_version', 'version', 'version.id = item.version_id')
      .left_join('wiprocurement.bom_item_emdm_extra', 'extra', 'extra.bom_id = version.bom_id and extra.source_item_id = item.source_item_id')
      .where('version_id = ?', id)
      .toParam()
