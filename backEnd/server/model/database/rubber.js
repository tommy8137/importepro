...
class Rubber {
  ...
  static async fetchMachineRateRelatedParam() {
  ...
  static async checkMachineRateExist(machineRate) {
    let sql = squel.select()
      .field('ton.ton', 'ton_name')
      .from('formula.rubber_machine_ton', 'ton')
      .where('UPPER(ton.ton) = ?', machineRate.toUpperCase())
  ...
  static async archiveMachineRate(client, machineRateIdList) {
    for(let machineRateId of machineRateIdList) {
      try {
        await Promise.all([
          commonModel.archiveData(client, 'formula.rubber_machine_ton', { id : machineRateId }),
        ])
      } catch (error) {
        throw new Error(error)
      }
  ...
  static async unblockMachineRate(client, machineRateIdList) {
    for(let machineRateId of machineRateIdList) {
      try {
        await Promise.all([
          commonModel.unblockData(client, 'formula.rubber_machine_ton', { id : machineRateId }),
        ])
      } catch (error) {
        throw new Error(error)
      }
