...
class Common {
  ...
  static async getScheduleDateForAdd(formulaType, productType = 1) {
    ...
    if (productType) {
      sql.where('sd1.product_type_id = ?', productType)
    } else {
  ...
  static async insertActivateDate(client, formulaType, nextDate, productTypeId) {
    ...
    const result = await client.query(sql.toString())
    return result
  }

  static async copyParameterByFormulaType(client, nextDate, copyDate) {
  ...
  static async updateDateByCopyDate(client, nextDate, copyDateId) {
    let sql = squel.update()
 ...
 */
   ...
   * 若存在則修改
   * @param {*} client
   * @param {String} id parameter_id
   * @param {Number} nextId
   * @param {Number} next 新增的value
...
此輸入將會去取得table中欄位名稱為process_time的parameterId，並將items的processTime set到parameter_value表的value上
   ...
   * @param {*} items 新增的資料
   */
  static async insertSingleTableItems(client, schema, items, returning = []) {
    if (items.length > 0) {
      // _.forEach(items, (item) => item.create_time = 'now()')
      ...
      if (returning.length > 0){
        _.forEach(returning, (returnKey) => {
          sqlInsert.returning(returnKey)
        })
      }

      try {
        let res = await client.query(sqlInsert.toParam())
        return res
      } catch (err) {
        logger.error('insertSingleTableItems error', schema, items)
        throw err
      }
    }
  }

  static async modifySingleTableDisable(client, schema, items) {
    try {
      ...
      await asyncForEach(items, async (item) => {
        ...
        let sql = squel.update()
          .table(schema)
          .set('disable_time', item.disable_time)
          .where('id = ?', item.id)

        await client.query(sql.toParam())
      })
    } catch(err){
      logger.error('modifySingleTableDisable error', schema, items)
      throw err
    }
  }
