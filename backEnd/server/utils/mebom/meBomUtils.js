 ...
 */
  ...
  static findSameBomItems(sourceData, reqObj, isEmdmProject = false) {
    let resData = _.filter(sourceData, (ori) => ori.part_number == reqObj.part_number && ori.part_name == reqObj.part_name && ori.id != reqObj.id)

    if (isEmdmProject) {
      resData = _.filter(resData, (same) => same.emdm_id == reqObj.emdm_id)
    }
    return resData
  }
  /**
   * 判斷 編輯的內容是否有相同料, 如果為同料則取最後一筆編輯資料同步之, 以下為判斷同料的條件
   * ePro: partnumber, partname
   * eMDM: partnumber, partname, emdmid
   * @param {*} reqItems
   * @param {*} bomItems
   * @param {*} ObjMappingKey
   * @param {*} isEmdmProject
   */
  static sortOutEditCostItems(reqItems, bomItems, ObjMappingKey, isEmdmProject = false) {
    let reqItemInfo = _.map(reqItems, (req) => {
      let reqId = req[ObjMappingKey.id]
      let bomInfo = _.find(bomItems, (item) => item.id == reqId)
      return {
        id: reqId,
        req : _.omit(req, ['id']),
        emdmId: bomInfo.emdm_id ? bomInfo.emdm_id : null,
        partnumber: bomInfo.part_number ? bomInfo.part_number : null,
        partname: bomInfo.part_name ? bomInfo.part_name : null,
      }
    })

    let groupItem = _.groupBy(reqItemInfo, i => isEmdmProject ? i.partname + i.partnumber + i.emdmId : i.partname + i.partnumber)
    let groupKey = Object.keys(groupItem)
    let res = []
    _.forEach(groupKey, (key) => {
      let lastItem = _.last(groupItem[key])
      res.push({
        id: lastItem.id,
        ...lastItem.req,
      })
