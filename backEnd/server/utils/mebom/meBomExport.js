...
class MeBomExport {
  ...
  async _parseBomTable(bomTableRes) {
    ...
    await asyncForEach(bomTableRes.bomItems, async (item) => {
      ...
      if (this.isExportPrice) {
        ...
        obj.part_name_highlight = isBaseRule && !isLastPrice && !isCleanSheet && !isOtherFill
        obj.space_remark = this._getSpaceRateRemark(item, obj)
      }
      bomTable.push(obj)
    })
    return bomTable
  }
  _getSpaceRateRemark(item, itemWithCost){
    let spaceRemark = ''
    if (!MeBomExport._hasQty(item, this.sku)) {
      spaceRemark = 'QTY為0'
    } else if(!MeBomExport._isChild(item)) {
      spaceRemark = '母階'
    } else if(!MeBomExport._isODM(item)) {
      spaceRemark = 'OEM/TBD'
    } else if(!MeBomExport._isFormate(item)) {
      spaceRemark = '無cleansheet'
    } else if(MeBomExport._otherFillMeRemarkRule(itemWithCost)) {
      spaceRemark = '新料(OtherFillMeRemark)'
    } else if(MeBomExport._cleanSheetCostRule(itemWithCost)) {
      spaceRemark = 'cleansheet有價格'
    } else if(MeBomExport._lastPriceRule(itemWithCost)) {
      spaceRemark = 'lastPrice有價格'
    }
    return spaceRemark
  }
  ...
  static _hasQty(item, sku = 'sku1') {
    if (item.hasOwnProperty(sku)) {
      return item[sku] != '0'
    } else {
      return item.sku_qty != 0
    }
  }
