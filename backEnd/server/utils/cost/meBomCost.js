...
/**
   * 取得ce運檢包
   * 20201007 Jira EP-919 CE Cost (U/P) 找不到CE運包時，不找Sourcer運包
   * @param {Object} bomItem
   */
function getCeShipping(bomItem){
  let ceShipping = 0//   (_.isNil(bomItem.shipping_check_cost)) ? (_.isNil(bomItem.sourcer_shipping)) ? 0 : Number(bomItem.sourcer_shipping) :  Number(bomItem.shipping_check_cost)
  if (!_.isNil(bomItem.shipping_check_cost)) {
    ceShipping = Number(bomItem.shipping_check_cost)
  }
