...
const RUBBER_FORMULA_TYPE = 'me_others_rubber'
class Rubber {
}
...
module.exports = {
  ...
  modifyMachineRate: async (nextId, items) => {
    let client = await new tsystemDB()
    try {
      let params = await rubberModel.fetchMachineRateRelatedParam()
  ...
  postMachineRate: async (machine, reqParams) => {
    let client = await new tsystemDB()
    try {
      let exist = await rubberModel.checkMachineRateExist(machine)
      if (exist) {
        throwApiError('F000401', errorCode.BAD_REQUEST)
      }
      // rubber_machine_ton
      let machine_ton = await commonModel.insertSingleTableItems(client, 'formula.rubber_machine_ton', [{
        ton: machine,
        create_time: 'now()',
      }])

      let params = await rubberModel.fetchMachineRateParam()
      let relatedList = _.map(params, (param) => {
        return {
          machine_ton_id: machine_ton[0].id,
          machine_param_id: param.param_id,
          create_time: 'now()',
        }
      })

      // rubber_machine_rate_related & parameter_value
      let relatedIdList = await commonModel.insertSingleTableItems(client, 'formula.rubber_machine_rate_related', relatedList, ['machine_param_id'])
      let scheduleList = await databaseUtils.getInsertScheduleDate(RUBBER_FORMULA_TYPE, 1)

      for (let index = 0; index < relatedIdList.length; index++) {
        const element = relatedIdList[index]
        let valueKey = params.find(p => p.param_id == element.machine_param_id)
        if (!valueKey) {
          throwApiError('[postMachineRate]:: can not find parameter key', errorCode.NOT_FOUND)
        }
        let value = reqParams[valueKey.param_name]

        await commonModel.addParameterValue(client, element.id, value, scheduleList, 'rubber_machine_rate_related')
      }

      await client.commit()
    } catch (err) {
      await client.rollback()
      logger.debug('Update table fail, postMachineRate function', err)
      throw err
    }
  },
  archiveMachineRate: async (machineRateIdList) => {
    let client = await new tsystemDB()

    try {
      await rubberModel.archiveMachineRate(client, machineRateIdList)
      await client.commit()
    } catch(err) {
      await client.rollback()
      logger.debug('Update table fail, archiveMachineRate function', err)
      throw err
    }
  },
  unblockMachineRate: async (machineRateIdList) => {
    let client = await new tsystemDB()

    try {
      await rubberModel.unblockMachineRate(client, machineRateIdList)
      await client.commit()
    } catch(err) {
      await client.rollback()
      logger.debug('Update table fail, unblockMachineRate function', err)
      throw err
    }
  },
}
