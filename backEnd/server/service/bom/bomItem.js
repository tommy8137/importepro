...
const updateSourcerCostItem = async (client, bomId, fromCurrency, uploadBomItemList, userID, role, isUpdateNull = false) => {
  ...
  let oriBomItems = await bomItemModel.getItemsByVerId(client, bomVersion[0].id)
  let editTypes = await bomItemModel.getBomEditType('bom_item_gerenal', ['sourcer'])
  let userInfo = await userModel.getUserInfo(userID)

  const assembleSourcerCostHistory = (bom_id, label, value, source_item_id, user_info) => {
    history.push({
      bom_id: bom_id.bom_id,
      field_type_id: _.find(editTypes, (type) => type.label == label).id,
      // field_type_name: _.find(edit_type, (type) => type.label == label).label_name,
      value,
      bom_version_id: bom_id.id,
      role,
      user_id:  user_info && user_info.length > 0 ? user_info[0].emplid : null,
      name_a:  user_info && user_info.length > 0 ? user_info[0].name_a : null,
  ...
  uploadBomItemList.forEach((item) => {
    let oriItem = _.find(oriBomItems, (i) => i.id == item.bom_item_id)
    let sameItems = meBomUtil.findSameBomItems(oriBomItems, oriItem, isEmdmProject)

    _.forEach(_.concat(oriItem, sameItems), (s) => {
      updatedItems.push({
        ...item,
        bom_item_id: s.id,
  ...
  updatedItems.forEach((item) => {
    ...
    for(let key of sourcerExchangeColumnList){
      ...
      if (!_.isNil(item[priceKey])) {
        ...
        if (isEmdmProject && oriItem[schemaKey] != item[priceKey]) {
          assembleSourcerCostHistory(bomVersion[0], priceKey, price, oriItem.source_item_id, userInfo)
        }
    ...
    if (isEmdmProject && item.hasOwnProperty('sourcer_remark') && item.sourcer_remark != oriItem.sourcer_remark) {
      assembleSourcerCostHistory(bomVersion[0], 'sourcer_remark', item.sourcer_remark, oriItem.source_item_id, userInfo)
    }
  })

  await bomItemModel.modifyBomItemBySourcerCostExcel(client, updatedItems, isUpdateNull)
  return history
}
...
module.exports = {
  ...
  confirmUploadBomSourcerCost: dbHelper.atomic(async (client, uploadTempId, user) => {
    ...
    // filter 出有被更新價格的item
    const filterEditCostItems = (reqItems, bomItems) => {
      let res = []

      _.forEach(reqItems, (req) => {
        let reqId = req.bom_item_id
        let oriItem = _.find(bomItems, (item) => item.id == reqId)

        let flag = _.some(sourcerExchangeColumnList, (key) => {
          let priceKey = key[0]
          let schemaKey = key[3]
          if ((req.hasOwnProperty(priceKey) && oriItem[schemaKey] != req[priceKey]) || (req.hasOwnProperty('sourcer_remark') && req.sourcer_remark != oriItem.sourcer_remark)) {
            return true
          }
        })
    ...
    let editedCostData = filterEditCostItems(uploadBomItemList, oriBomItems)
    let updateData = meBomUtil.sortOutEditCostItems(editedCostData, oriBomItems, { id: 'bom_item_id' }, isEmdmProject)

    let sourcerCostHistory = await updateSourcerCostItem(client, bomId, fromCurrency, updateData, userID, role)
    await bomItemModel.deleteSourcerCostExcel(client, uploadTempId)

    if (isEmdmProject) await bomItemModel.insertBomEditHistory(client, sourcerCostHistory)
  }),
  ...
  updateBomItemCost:dbHelper.atomic(async (client, bomId, data, userId) => {
    ...
    let isEmdmProject = await emdmBomUtil.isEmdmProject(bomId)
    let bomVersionId = await bomItemModel.getItemVersionByBomId(bomId)
    let originDataset = await bomItemModel.getItemsByVerId(client, bomVersionId[0].id)
    let editType = await bomItemModel.getBomEditType('bom_item_gerenal', ['spa', 'ce'])
    let userInfo = await userModel.getUserInfo(userId)
    ...
    let assembleHistory = (historyArr, bom_id, label, source_item_id, user_info, value) => {

      historyArr.push({
        bom_id: bom_id.bom_id,
        field_type_id: _.find(editType, (type) => type.label == label).id,
        // field_type_name: _.find(editType, (type) => type.label == label).label_name,
        value: value,
        bom_version_id: bom_id.id,
        role: 'CE',
        user_id: user_info && user_info.length > 0 ? user_info[0].emplid : null,
    ...
    // edit 的資料中如果有同料的話, 只取最後一筆資訊(CE_cost & Sourcer Cost)
    let updateData = meBomUtil.sortOutEditCostItems(data, originDataset, { id: 'id'}, isEmdmProject)

    for(let i = 0; i < updateData.length; i++){
      let bomItem = updateData[i]
      let oriBomItem = _.find(originDataset, (ori) => ori.id == bomItem.id)
      let sameBomItems = meBomUtil.findSameBomItems(originDataset, oriBomItem, isEmdmProject)
      let updateBomItems = _.concat(oriBomItem, sameBomItems)
      ...
      if(typeof bomItem.suggestion_cost_type === 'string'){
        ...
        if (isEmdmProject) {
          _.forEach(updateBomItems, (s) => assembleHistory(suggCostHistory, bomVersionId[0], 'ce_cost_up', s.source_item_id, userInfo, null))
        }
      }
      ...
      if(Object.keys(reqObj).length > 0) {
        ...
        await asyncForEach(updateBomItems, async (s) => await bomItemModel.updateBomItem(client, s.id, reqObj))

        if (isEmdmProject) {
          Object.keys(reqObj).forEach((obj) => {
            // 非null 的值皆轉型為string
            let value = reqObj[obj] ? reqObj[obj].toString() : reqObj[obj]
            _.forEach(updateBomItems, (s) => assembleHistory(history, bomVersionId[0], obj, s.source_item_id, userInfo, value))
          })
        }
      }
    }

    return { history, suggCostHistory }
  }),
  insertItemCostEditHistory: dbHelper.atomic(async (client, bomID, userID, sourcerHistory = [], ceCostHistory = { history: [], suggCostHistory: [] }) => {
    let history = []
    let suggCostHistory = ceCostHistory.suggCostHistory

    if (suggCostHistory.length > 0) {
      let fullBom = await getBomItem(bomID, userID, 'all', 'sku0')
      _.forEach(suggCostHistory, (sugg) =>{
        let item = _.find(fullBom.bomItems, (bom) => bom.source_item_id == sugg.source_item_id)
        sugg.value = item.ce_cost_up
      })
    }

    history = _.concat(ceCostHistory.history, suggCostHistory, sourcerHistory)
  ...
  updateBomItemSourcerCost: dbHelper.atomic(async (client, bomId, data, userId) => {
    ...
    let bomVersion = await bomItemModel.getItemVersionByBomId(bomId)
    let oriBomItems = await bomItemModel.getItemsByVerId(client, bomVersion[0].id)
    let updateData = meBomUtil.sortOutEditCostItems(data, oriBomItems, { id: 'id' }, isEmdmProject)

    // check all data include bomItemid
    _.forEach(updateData, (v, idx) => {
      let _id = v.id
      v.bom_item_id = v.id
    ...
    if (updateColumn.length) {
      return await updateSourcerCostItem(client, bomId, 'USD', updateData, userId, role, true)
    }
    return []
  }),
  /**
