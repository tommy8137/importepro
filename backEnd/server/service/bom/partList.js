...
*/
  ...
  updatePartList:dbHelper.atomic(async (client, bomItemId, data, userID) =>{
    ...
    let assembleHistory = (edit_type_list, reqArr, bom_id, source_item_id, user_info) => {
    ...
    if (!_.isEmpty(data.partlistValue)) {
      ...
      if (isEmdmProject && ceParameters) {
        ...
        let oriCeParametersTab = oriPartlistValue.formData ? oriPartlistValue.formData[ceParametersTabKey] : null

        let reqObj = {}
        _.forEach(ceParametersKeyList, (ceParametersKey) => {
          let reqCeParameters = ceParametersTab[ceParametersKey]
          let oriCeParameters = oriCeParametersTab ? oriCeParametersTab[ceParametersKey] : {}
        ...
        history = assembleHistory(editType, reqObj, bomVersion[0], bomItem[0].source_item_id, userInfo)
      }
