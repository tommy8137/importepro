...
class BomManager {
  ...
  static async bomApproval(bomID, userID, copyCostBomID) {
    ...
    let sourcerPermissionList = null

    if (isEmdmProject) {
      checkApproveRes = await emdmBomUtil.checkAllowApprove(bomID, userID)
      sourcerPermissionList = await this.getSourcerPermission(bomID)
    ...
    if (checkApproveRes) {
      if (isEmdmProject && sourcerPermissionList && sourcerPermissionList.hasOwnProperty('sourcer_permission') && sourcerPermissionList.sourcer_permission.length === 0) {
        logger.error('[bomManager][bomApproval] error : ', 'Sourcer mail list is empty')
        throwApiError('C000601', errorCode.NOT_FOUND)
      }
          ...
          if (param.label_name === originParam.label_name && param.group_name === originParam.group_name) {
            ...
            if (changeFlag) {
              history.push({
                ...
                role: 'CE',
                user_id: userID,
                name_a: userInfo && userInfo.length > 0 ? userInfo[0].name_a : null,
  ...
  static async sendMailToSourcer(bomId){
    try {
      ...
      if(sourcerPermissionList.length === 0){
        logger.error('[bomManager][sendMailToSourcer] error : ', 'Sourcer mail list is empty')
        throwApiError('C000601', errorCode.NOT_FOUND)
      }
      const bomProjectInfo = await this._getEmailBomProjectInfo(bomId)
      const bomItemListDomain = `${emdmBomItemListBaseDomain}/${bomId}`
      ...
      logger.error('[bomManager][sendMailToSourcer] error : ', error)
      throw error
    }
  }
