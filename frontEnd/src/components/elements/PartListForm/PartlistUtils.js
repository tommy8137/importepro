...
function getLayoutField(layout, key) {
  return traverse(layout).reduce(function r(acc, field) {
    if (field?.key === key) {
      // const paths = getValuePath(layout, this.path);
      this.stop(true);
      return field;
      // return { field, formDataPath: paths, layoutPath: this.path };
    }
    return acc;
  }, null);
}

function getFieldDefaultValue(layout, field, partItemInfo, layoutPath, formData) {
  if (field?.key) {
    const { fieldType, replaceBy, func, default: defaultValue, defaultCalc, defaultValues } = field;
    let value = defaultValue;
    if (defaultCalc && defaultValues) {
      value = safeEval(defaultCalc)(defaultValue, partItemInfo, defaultValues);
    }
    if (fieldType === 'input' && replaceBy && func) {
      const params = replaceBy.map(k => {
        const dependField = getLayoutField(layout, k);
        const val = getFieldDefaultValue(layout, dependField, partItemInfo);
        if (val !== null) {
          return val;
        }
        return undefined;
      });
      value = safeEval(func)(...params);
    }
    return value;
  }
  return null;
  ...
  return traverse(layout).reduce(function r(acc, item) {
    ...
    try {
      if (item.defaultCalc) {
        ...
        defaultValue = safeEval(item.defaultCalc)(item.default, partItemInfo, defaultValues);
      }
    } catch (e) { console.log(e); }

    // 處理經由replaceBy的default值
    if (item.fieldType === 'input' && item.replaceBy && item.func) {
      const params = item.replaceBy.map(k => {
        const dependField = getLayoutField(layout, k);
        const val = getFieldDefaultValue(layout, dependField, partItemInfo);
        if (val !== null) {
          return val;
      ...
      defaultValue = safeEval(item.func)(...params);
    }
