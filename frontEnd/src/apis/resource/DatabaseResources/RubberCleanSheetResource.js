...
const RubberCleanSheetResource = {
  ...
  putMachineRate: data => {
    if (debugStatus.putMachineRate) {
      return CommonUtils.fakeApiHelper(1000, 'success', true);
    }
    return ApiService.put('/database/rubber/modify/machineRate', { data });
  },

  addMachineRate: data => {
    if (debugStatus.addMachineRate) {
      return CommonUtils.fakeApiHelper(1000, 'success', true);
    }
    return ApiService.post('/database/rubber/machineRate', { data });
  },

  archiveMachineRate: data => {
    if (debugStatus.archiveMachineRate) {
      return CommonUtils.fakeApiHelper(1000, 'success', true);
    }
    return ApiService.post('/database/rubber/archive/machineRate', { data });
  },
  unArchiveMachineRate: data => {
    if (debugStatus.unArchiveMachineRate) {
      return CommonUtils.fakeApiHelper(1000, 'success', true);
    }
    return ApiService.post('/database/rubber/unblock/machineRate', { data });
  },
